#!/bin/bash 
 ...
 docker run -itd --name host1 --network="none" --privileged -v shared:/codes --workdir /codes ernestovaz/net
 alias host1="docker exec -it host1 /bin/bash" 

 
 echo "Start Host Container host2" 
 docker run -itd --name host2 --network="none" --privileged -v shared:/codes --workdir /codes ernestovaz/net
...
#docker exec sw1 ethtool -K sw1-p1-host1-p1 tx off tx off 
docker exec sw1 ip link set sw1-p2-host2-p1 promisc on 
docker exec host2 ip link set host2-p1-sw1-p2 promisc on 
#docker exec sw1 ethtool -K sw1-p2-host2-p1 tx off tx off 
...
docker exec sw1 sh -c 'nohup simple_switch  --thrift-port 50001  -i 1@sw1-p1-host1-p1 -i 2@sw1-p2-host2-p1 int.json --log-console >> /tmp/switch.log &' 
